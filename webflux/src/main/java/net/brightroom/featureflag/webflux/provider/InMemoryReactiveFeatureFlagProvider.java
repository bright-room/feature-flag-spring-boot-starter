...
/**
 * An implementation of {@link ReactiveFeatureFlagProvider} that stores feature flag configurations
 * in memory using a {@link Map}.
 *
...
public class InMemoryReactiveFeatureFlagProvider implements ReactiveFeatureFlagProvider {
  ...
  /**
   * Returns whether the specified feature is enabled as a {@link Mono}.
   *
   * <p><b>Fail-closed behavior (default):</b> If {@code featureName} is not present in the feature
   * map, this method emits the value of {@code defaultEnabled} (defaults to {@code false}). With
   ...
   * @return a {@link Mono} emitting {@code true} if the feature is explicitly enabled, {@code
   *     false} if explicitly disabled or not configured (with default fail-closed behavior)
   */
  @Override
  ...
  /**
   * Constructs an instance of {@code InMemoryReactiveFeatureFlagProvider} with the provided feature
   * flags and default enabled status.
   *
   ...
   */
  public InMemoryReactiveFeatureFlagProvider(
